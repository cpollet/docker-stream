...
func Run(runConfiguration *RunConfiguration) {
	executionContext := context.HandleSigint()

	config, err := configuration.Read(runConfiguration.Filename)
	if err != nil {
		panic(err)
	}
	...
	if config.Name == "" {
		config.Name = path.Base(runConfiguration.WorkDir)
	}
	...
	ctx := &context.Context{
		...
		WorkDir:          runConfiguration.WorkDir,
